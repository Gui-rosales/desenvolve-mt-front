...
export class DetailsPage {
  ...
  public get personPhoto() {
    return this.page.locator('img[alt*="Foto de"]');
  }
  ...
  public get statusBadge() {
    return this.page.locator('.bg-primary, .bg-destructive, .bg-yellow-500');
  }

  // Métodos de navegação
  async navigateToPersonDetails(personId: number) {
    await this.page.goto(`/${personId}`);
  }
  ...
  async getDisappearanceDate(): Promise<string> {
    const dateElement = await this.page.locator('text=Data do Desaparecimento').locator('..').locator('.text-lg');
    const dateText = await dateElement.textContent();
    return dateText || '';
  }

  async getDisappearanceLocation(): Promise<string> {
    const locationElement = await this.page.locator('text=Local do Desaparecimento').locator('..').locator('.text-foreground');
  ...
  async fillOcorrenciaForm(data: {
    ...
    await this.page.locator('[role="grid"]').locator(`text=${data.data.getDate()}`).click();
  }

  async selectDateInOcorrenciaModal(date: Date) {
    await this.page.getByTestId('date-picker-button').click();
    await this.page.locator('[role="grid"]').locator(`text=${date.getDate()}`).click();
  }
  ...
  async getOcorrenciasCount(): Promise<number> {
    const ocorrencias = await this.page.locator('[data-testid="ocorrencia-item"]').count();
    return ocorrencias;
  }

  async isOcorrenciaVisible(index: number): Promise<boolean> {
    const ocorrencia = await this.page.locator('[data-testid="ocorrencia-item"]').nth(index);
  ...
  async waitForPersonDetailsToLoad() {
    await this.page.waitForSelector('[data-testid="person-details"]', { state: 'visible' });
  }

  async waitForOcorrenciasToLoad() {
    await this.page.waitForSelector('[data-testid="ocorrencia-item"]', { state: 'visible' });
  }

  async expectLoadingToBeVisible() {
    await expect(this.page.getByText('Carregando informações...')).toBeVisible();
  }

  async expectErrorToBeVisible() {
    await expect(this.page.getByText('Pessoa não encontrada ou erro ao carregar informações')).toBeVisible();
  }
  ...
  async isAdditionalInfoVisible(): Promise<boolean> {
    const infoElement = await this.page.locator('text=Informações Adicionais').first();
    return await infoElement.isVisible();
  }

  async isVestimentasVisible(): Promise<boolean> {
    const vestimentasElement = await this.page.locator('text=Vestimentas').first();
    return await vestimentasElement.isVisible();
  }

  async getAdditionalInfoText(): Promise<string> {
    const infoElement = await this.page.locator('text=Informações Adicionais').first().locator('..').locator('.text-foreground');
    const text = await infoElement.textContent();
    return text || '';
  }

  async getVestimentasText(): Promise<string> {
    const vestimentasElement = await this.page.locator('text=Vestimentas').first().locator('..').locator('.text-foreground');
