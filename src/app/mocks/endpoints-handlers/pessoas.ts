...
function generateFakePessoasResponse(params: any) {
  const pessoas = Array.from({ length: 12 }, (_, i) => ({
    ...
    ultimaOcorrencia: {
      ...
      localDesaparecimentoConcat: faker.location.city() + ', ' + faker.location.state(),
      dataLocalizacao: faker.helpers.maybe(() => faker.date.recent().toISOString()),
      ocorrenciaEntrevDesapDTO: {
        informacao: faker.lorem.paragraph(),
        vestimentasDesaparecido: faker.lorem.sentence()
      },
      listaCartaz: Array.from({ length: faker.number.int({ min: 0, max: 3 }) }, () => ({
        urlCartaz: faker.image.url(),
        tipoCartaz: faker.helpers.arrayElement(['Cartaz Principal', 'Cartaz Secundário'])
      }))
    }
  }));
