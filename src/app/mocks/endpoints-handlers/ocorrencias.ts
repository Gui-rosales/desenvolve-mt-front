...
// Função para gerar dados fake de ocorrências
...
export const ocorrenciasHandlers = [
  // GET /ocorrencias/informacoes-desaparecido
  http.get(`${BASE_URL}/informacoes-desaparecido`, ({ request }) => {
    const url = new URL(request.url);
    const ocorrenciaId = url.searchParams.get('ocorrenciaId');
    
    if (!ocorrenciaId) {
      return HttpResponse.json({ error: 'ocorrenciaId é obrigatório' }, { status: 400 });
    }
  ...
  http.post(`${BASE_URL}/informacoes-desaparecido`, async ({ request }) => {
    ...
    if (!params.informacao || !params.descricao || !params.data || !params.ocoId) {
